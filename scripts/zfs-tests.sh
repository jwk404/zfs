...
#
# If ZTS_OBJECT_STORE is not set and DISKS have not been provided, a basic
# file or loopback based devices must be created for the test suite to use.
#

if [ -n "$ZTS_OBJECT_STORE" ]; then
	# No need to specify disks if we're using object storage, but
	...
	# traces for zfs-object-agent service crash.
	#
	export RUST_BACKTRACE=1

	#
	# Ensure that the log file doesn't exist before the run and then
	# start the agent.
	#
	if [ -f "$ZOA_LOG" ]; then
		sudo rm -f $ZOA_LOG
	fi
	sudo /sbin/zfs_object_agent -vv \
